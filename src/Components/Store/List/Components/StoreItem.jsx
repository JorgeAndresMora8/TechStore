...
export default function StoreItem({ product }) {
    ...
    return (<div className='container-item'>
        ...
        <div style={{width: '100%', height: 'auto', display: 'flex', justifyContent: 'center', alignItems: 'center' , padding: '5px 10px'}}>
        <Link to={`/store/${product.id}`} className="btn-go-detail">buy now</Link>
        </div>
    </div>)
}